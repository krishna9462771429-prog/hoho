/*
  ...
  1. Problem
    - user_profiles.id was TEXT but Supabase Auth uses UUID
    - Foreign keys are failing due to type mismatch
  
  2. Solution
    - Drop and recreate user_profiles with id as UUID
    - Update all foreign key references
    - Ensure proper auth.users integration
*/

-- Drop existing policies and foreign keys
...
DROP TABLE IF EXISTS ai_fallbacks;
DROP TABLE IF EXISTS monitoring_logs;
DROP TABLE IF EXISTS ticker_services;
...
-- Recreate user_profiles with correct UUID type
...
CREATE TABLE user_settings (
  ...
  use_personal_keys BOOLEAN DEFAULT false,
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'apis' AND policyname = 'Users can view own apis') THEN
    CREATE POLICY "Users can view own apis" ON apis FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'apis' AND policyname = 'Users can insert own apis') THEN
    CREATE POLICY "Users can insert own apis" ON apis FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'apis' AND policyname = 'Users can update own apis') THEN
    CREATE POLICY "Users can update own apis" ON apis FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'apis' AND policyname = 'Users can delete own apis') THEN
    CREATE POLICY "Users can delete own apis" ON apis FOR DELETE TO authenticated USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'api_logs' AND policyname = 'Users can view own logs') THEN
    CREATE POLICY "Users can view own logs" ON api_logs FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'api_logs' AND policyname = 'Users can insert own logs') THEN
    CREATE POLICY "Users can insert own logs" ON api_logs FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'workflows' AND policyname = 'Users can view own workflows') THEN
    CREATE POLICY "Users can view own workflows" ON workflows FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'workflows' AND policyname = 'Users can insert own workflows') THEN
    CREATE POLICY "Users can insert own workflows" ON workflows FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'workflows' AND policyname = 'Users can update own workflows') THEN
    CREATE POLICY "Users can update own workflows" ON workflows FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'workflows' AND policyname = 'Users can delete own workflows') THEN
    CREATE POLICY "Users can delete own workflows" ON workflows FOR DELETE TO authenticated USING (auth.uid() = user_id);
  END IF;
END $$;

-- ticker_services
CREATE TABLE ticker_services (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  url TEXT NOT NULL,
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ticker_services' AND policyname = 'Users can view own ticker services') THEN
    CREATE POLICY "Users can view own ticker services" ON ticker_services FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ticker_services' AND policyname = 'Users can insert own ticker services') THEN
    CREATE POLICY "Users can insert own ticker services" ON ticker_services FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ticker_services' AND policyname = 'Users can update own ticker services') THEN
    CREATE POLICY "Users can update own ticker services" ON ticker_services FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ticker_services' AND policyname = 'Users can delete own ticker services') THEN
    CREATE POLICY "Users can delete own ticker services" ON ticker_services FOR DELETE TO authenticated USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'monitoring_logs' AND policyname = 'Users can view own monitoring logs') THEN
    CREATE POLICY "Users can view own monitoring logs" ON monitoring_logs FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'monitoring_logs' AND policyname = 'Users can insert own monitoring logs') THEN
    CREATE POLICY "Users can insert own monitoring logs" ON monitoring_logs FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fallbacks' AND policyname = 'Users can view own ai fallbacks') THEN
    CREATE POLICY "Users can view own ai fallbacks" ON ai_fallbacks FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fallbacks' AND policyname = 'Users can insert own ai fallbacks') THEN
    CREATE POLICY "Users can insert own ai fallbacks" ON ai_fallbacks FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'provider_usage' AND policyname = 'Users can view own provider usage') THEN
    CREATE POLICY "Users can view own provider usage" ON provider_usage FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'provider_usage' AND policyname = 'Users can insert own provider usage') THEN
    CREATE POLICY "Users can insert own provider usage" ON provider_usage FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'analytics_snapshots' AND policyname = 'Users can view own analytics') THEN
    CREATE POLICY "Users can view own analytics" ON analytics_snapshots FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'analytics_snapshots' AND policyname = 'Users can insert own analytics') THEN
    CREATE POLICY "Users can insert own analytics" ON analytics_snapshots FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'analytics_snapshots' AND policyname = 'Users can update own analytics') THEN
    CREATE POLICY "Users can update own analytics" ON analytics_snapshots FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cache_entries' AND policyname = 'Users can view own cache') THEN
    CREATE POLICY "Users can view own cache" ON cache_entries FOR SELECT TO authenticated USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cache_entries' AND policyname = 'Users can insert own cache') THEN
    CREATE POLICY "Users can insert own cache" ON cache_entries FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cache_entries' AND policyname = 'Users can update own cache') THEN
    CREATE POLICY "Users can update own cache" ON cache_entries FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cache_entries' AND policyname = 'Users can delete own cache') THEN
    CREATE POLICY "Users can delete own cache" ON cache_entries FOR DELETE TO authenticated USING (auth.uid() = user_id);
  END IF;
END $$;
